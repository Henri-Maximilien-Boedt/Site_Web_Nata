...
3. **Mobile-oriented** — coder pour 375px d'abord, élargir avec `sm:` `md:` `lg:`
...
│       └── Badge.jsx
├── lib/
│   ├── supabase.js            → createBrowserClient (anon key) — @supabase/auth-helpers-nextjs
...
> ⚠️ `@supabase/auth-helpers-nextjs@0.15.0` est la version finale dépréciée — elle re-exporte
> `@supabase/ssr`. Les anciennes fonctions (`createMiddlewareClient`, `createBrowserSupabaseClient`)
> n'existent plus. Utiliser `createServerClient` / `createBrowserClient`.

```js
// middleware.js — API correcte
...
export async function middleware(req) {
  ...
  const supabase = createServerClient(
    ...
    {
      cookies: {
        getAll() { return parseCookieHeader(req.headers.get("Cookie") ?? "") },
        setAll(cookiesToSet) {
...
- [x] Phase 1 — Setup : `create-next-app`, Supabase, `.env.local`, layout global ✅
...
```bash
npm run dev    # ← toujours utiliser ça, PAS npx next dev
```
...
### 2026-03-03 — Phase 1 complète

**Fichiers créés :**
- `styles/globals.css` — variables CSS NATA Bar + noise texture + reset + scrollbar custom
- `styles/floorplan.css` — plan de salle CSS pur (statuts free/pending/reserved/small, drag mode)
- `tailwind.config.js` — couleurs NATA Bar intégrées (`bg-main`, `bg-card`, `accent`, `cream`…)
- `pages/_document.js` — `lang="fr"`, Google Fonts (Bebas Neue + Manrope 400/500/700/800)
- `pages/_app.js` — Header/Footer injectés automatiquement sauf sur `/admin/*`
- `lib/supabase.js` — singleton `createBrowserClient` (anon key)
- `lib/supabaseAdmin.js` — `createClient` service role, uniquement pour `pages/api/`
- `lib/brevo.js` — `sendEmail({ to, toName, subject, html })`
- `lib/dateUtils.js` — `OPENING_HOURS`, `timesOverlap()`, `getSlotsForDate()`, `isOpenDay()`, `formatDateFR()`
- `middleware.js` — protection `/admin/*` avec redirect `/login?redirect=...`
- `components/layout/Header.jsx` — nav desktop + burger mobile animé, lien actif, ombre au scroll
- `components/layout/Footer.jsx` — 3 colonnes (identité + adresse, horaires, navigation)
- `components/layout/AdminLayout.jsx` — sidebar desktop + drawer mobile, logout Supabase
- `pages/index.js` — hero placeholder

**Bug corrigé :**
- `@supabase/auth-helpers-nextjs@0.15.0` ne contient plus `createBrowserSupabaseClient` ni `createMiddlewareClient` — remplacés par `createBrowserClient` et `createServerClient` (nouvelle API `@supabase/ssr`).
